...
const CartSideBar = () => {
    ...
    return (
        <>
            {isSidebarOpen && (
                <div className="absolute w-full h-full bg-black z-20 bg-opacity-50 overflow-hidden font-bodyFont">
                    <div className={`overflow-auto w-[30%] min-w-[300px] h-full bg-white right-0 absolute ${isAnimating ? 'animate-sideBarClose' : 'animate-sideBarOpen'}`}>
                                        ...
                                        : <div>
                                            {
                                                cartItems?.map((item, index) => {
                                                    return (
                                                        <div key={index} className='flex justify-between py-[10px] border-t font-bodyFont'>
                                                            ...
                                                            <div className='w-[50%] flex justify-between flex-col'>
                                                                ...
                                                                <div className='bg-black text-white p-[10px] w-max text-[13px] text-center cursor-pointer' onClick={() => { setIsModalOpen(true); setproductToUpdateQuantity(item); }}>Update Quantity</div>
                                                            </div>
            ...
            <Modal isOpen={isModalOpen} closeModal={() => setIsModalOpen(false)}>
                ...
                <div className='bg-black text-white p-[10px] w-max text-[13px] text-center cursor-pointer' onClick={() => quantityOfProduct(productToUpdateQuantity)}>{quantityUpdateLoading ? <div className='w-[42px] flex items-center justify-center'><ButtonLoader /></div> : "Update"}</div>
            </Modal>
        </>
    );
};
